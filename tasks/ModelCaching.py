...
def update_models(cache_dp_to_update=None):
    ...
    # ----------------------------------------------------------------------------------
    # ----------------------------------------------------------------------------------
    wp.ThisJob.logprint(' '.join([f"{n}x{t[:5]}" for t,n in zip(*np.unique(list(map(lambda I: type(I).__name__,wp.si._consolidate_cached_instances())), return_counts=True))]))
    # ----------------------------------------------------------------------------------
    # ----------------------------------------------------------------------------------
...
def clear_model_dp_from_wpipe_cache():
    # CLEAR CACHE, bit of a clumsy fix there...-----------------------------------------
    wp.ThisJob.logprint(' '.join(["Before clear"]+[f"{n}x{t[:5]}" for t,n in zip(*np.unique(list(map(lambda I: type(I).__name__,wp.si._consolidate_cached_instances())), return_counts=True))]))
    ...
    del _tmp_opt, _temp
    gc.collect()
    wp.ThisJob.logprint(' '.join(["After clear"]+[f"{n}x{t[:5]}" for t,n in zip(*np.unique(list(map(lambda I: type(I).__name__,wp.si._consolidate_cached_instances())), return_counts=True))]))
    # ----------------------------------------------------------------------------------
